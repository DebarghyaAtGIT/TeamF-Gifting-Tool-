...
export class UserProfileComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute,private _userService:GiftToolService
    ,private _orderService:OrderService, private _snackBar:MatSnackBar, private _router: Router) { }

  getUser(email:string): void{
    this._userService.getAll(email)
    .subscribe(
      (user: any) => {
        ...
        if(this.isAuth)
           this.orderHistory(this.currentUser.pkUserId);
      },
      (error: any) => {
        console.log(error);
      }
  ...
  public orderHistory(id:number): void{
    this._orderService.getOrderHistory(id)
    .subscribe(
      (order:any) => {
