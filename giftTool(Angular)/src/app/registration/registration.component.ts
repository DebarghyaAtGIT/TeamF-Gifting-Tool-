...
export class RegistrationComponent {
  ...
  constructor(
    private _registrationservice: GiftToolService,
  ...
  public onSubmit(): void {
    if (
      ...
      this.signUpForm.controls['cpass'].value
    ) {
      this.isValid = true;
    }

    if (!this.signUpForm.controls['checkBox'].dirty) this.isValid = false;
  ...
  //Error handling and Creating user account
  public getRequiredData(data: User[]) {
    ...
    if (
      ...
      this._registrationservice.addUser(newUser).subscribe((data) => {
        this._registrationservice.getLoggedUser(data).subscribe(
          (res) => {
    ...
    } else alert('This is not a valid form');
  }

  //Storing Users' data in local Storage
